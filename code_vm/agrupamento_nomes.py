...
def make_as_matrix(names_s):
    ...
    # Supondo que names_s seja a série de nomes
    names_array = np.array(names_s, dtype=str)

    # Crie uma matriz de comparação
    starts_with_matrix = np.identity(len(names_array), dtype=int)

    # Preencha a matriz com 1 onde nome_1 começa com nome_2
    for i, nome_1 in enumerate(names_array):
        #for j in range(i, len(names_array)):
            #nome_2 = names_array[j]
        for j, nome_2 in enumerate(names_array):
            if i==j:
                starts_with_matrix[i, j] = 1
            elif len(nome_1) >= len(nome_2) and nome_1.startswith(nome_2):
            #if len(nome_1) >= len(nome_2) and nome_1.startswith(nome_2):
                starts_with_matrix[i, j] = 1
