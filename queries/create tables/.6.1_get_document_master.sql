...
where 1=1
and reference_month = DATE_ADD(DATE_TRUNC(ref_month, MONTH), interval -1 day)
)
...
select 
LAST_DAY(DATE_ADD(reference_month, interval 1 day), MONTH) reference_month, 
merchant_market_hierarchy_id,
subs_asterisk,
numero_inicio,
nome_master,
...
, active_base_stone as (
  ...
  CROSS JOIN (select distinct reference_month from names_city_master)
  WHERE 1=1
...
CAST(null as STRING) nome_cpf,
CAST(null as STRING) nome_cnpj,
--IF(length(document) = 11, 1, 0) qtd_cpfs,
--IF(length(document) = 14, 1, 0) qtd_cnpjs,
CAST(null as STRING) nome_cpf_brasil,
CAST(null as STRING) cpf_brasil,
True bom_resultado,
