--CREATE OR REPLACE TABLE `dataplatform-prd.master_contact.aux_tam_acquirers` PARTITION BY reference_date AS 


DECLARE ref_month  DATE DEFAULT '{{ref_month}}';
INSERT INTO `dataplatform-prd.master_contact.aux_tam_acquirers` 
...
inner join (select distinct tam_id, reference_date reference_month from `dataplatform-prd.master_contact.aux_tam_pos_python`) using(tam_id, reference_month)
where subs_asterisk = 'Outros'
...
null issue_get,
from `dataplatform-prd.master_contact.tmp_mid_de42_acquirer_model_v2`
...
FROM `dataplatform-prd.master_contact.aux_tam_tam_acquirers` 
