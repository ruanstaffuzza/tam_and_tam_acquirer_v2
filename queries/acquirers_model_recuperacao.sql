...
DECLARE ref_month_max  DATE DEFAULT '{{ref_month_max}}';
...
and date(reference_month) <= ref_month_max
...
inner join (select distinct tam_id from `dataplatform-prd.master_contact.aux_tam_pos_python`) using(tam_id)
